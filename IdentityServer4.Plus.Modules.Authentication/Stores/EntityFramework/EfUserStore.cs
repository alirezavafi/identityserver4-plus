...
namespace SSO.Identity.Stores.EntityFramework
{
    public class EfUserStore : ApplicationUserStoreBase
    {
        ...
        public override Task<List<ApplicationUser>> FindByPhoneNumberAsync(string mobile,
            CancellationToken cancellationToken = default)
        {
            return dbContext.Users.Where(x => x.MobileNumber == mobile).ToListAsync();
        }

        public override Task<List<ApplicationUser>> FindByAnyIdentifierAsync(string identifier,
            CancellationToken cancellationToken = default)
        {
            return dbContext.Users.Where(x =>
                    x.MobileNumber == identifier
                    || x.Email == identifier
                    || x.UserIdentityNumber == identifier)
                .ToListAsync();
        }
