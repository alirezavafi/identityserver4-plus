...
namespace SSO.Identity
{
    ...
    {
        public static IServiceCollection AddApplicationIdentity(this IServiceCollection services, IConfiguration configuration, IHostEnvironment env)
        {
            ...
            var builder = services.AddIdentityServer(
                    options =>
                    {
                        ...
                        {
                            CookieLifetime = TimeSpan.FromMinutes(int.Parse(configuration["Settings:SsoLifeTimeInMins"] ?? "120")),
                            CookieSlidingExpiration = bool.Parse(configuration["Settings:SsoIsSlidingExpiration"] ?? "false"),
                ...
                {
                    options.ConfigureDbContext = b => b.UseSqlServer(connectionString, sql => sql.MigrationsAssembly("Api").EnableRetryOnFailure(3));
                ...
                .AddOperationalStore(options =>
                {
                    options.ConfigureDbContext = b =>
                        b.UseSqlServer(connectionString, sql => sql.MigrationsAssembly("Api").EnableRetryOnFailure(3));
            ...
            {
                var signCertPath = Path.Combine(env.ContentRootPath, configuration["TokenSigningCertificatePath"]);
                Console.WriteLine("Setting sign cert:" + signCertPath);
                var rsaCertificate = new X509Certificate2(signCertPath, configuration["TokenSigningCertificationPassword"]);
